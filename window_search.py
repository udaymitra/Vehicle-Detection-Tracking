...
def search_windows_fast(img, y_start, y_stop, clf, scaler, scale=1,
    ...
    for xb in range(nxsteps):
        for yb in range(nysteps):
            ...
            hog_features_per_channel = [hog_feat1, hog_feat2, hog_feat3]
            ...
            if hog_feat == True:
                if hog_channel == 'ALL':
                    ...
                    test_features.append(hog_features_per_channel[hog_channel])

            test_features = np.concatenate(test_features)
            test_features = scaler.transform(np.array(test_features).reshape(1, -1))
